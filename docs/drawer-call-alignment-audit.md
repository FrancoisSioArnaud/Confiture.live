...
| Drawer ciblé par `visualIndex` global projeté | Conforme à migrer | `CallDrawer` doit recevoir le `visualIndex` global et utiliser le `resolvedRow` correspondant fourni par la projection ; ne pas lire les cards par rang local de colonne. |
