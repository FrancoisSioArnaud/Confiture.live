...
Ce document remplace l’ancienne spec de réorder. Il est la référence canonique et normative pour l’ordre des cards du tableau Confiture.live.

Règle d’autorité : si un autre document, un ancien commentaire de code ou une ancienne implémentation contredit ce fichier, ce fichier gagne. Les autres docs peuvent préciser le branchement, les payloads ou les tests, mais ne peuvent pas redéfinir le comportement du resolver.

Documents complémentaires normatifs uniquement dans leur périmètre :

```txt
- docs/projection-engine-spec.md : replay, projection, branchement du resolver.
- docs/event-payloads-reference.md : payloads et noms d’events.
- docs/testing-strategy.md : invariants, fixtures et definition of done test.
- docs/order-resolution-golden-fixtures.md : fixtures golden exactes.
```
...
Les noms canoniques V0 sont fermés pour éviter les synonymes dans le code :
...
plateau_played
plateau_unplayed
visualIndex        // valeur UI/affichage global
resolvedRow        // valeur logique du resolver
playedResolvedRow  // row figée lors d’un plateau_played
```

Noms obsolètes ou interdits dans le chemin critique du resolver :
...
Catalogue fermé des warnings V0 : aucun autre `type`/`reason` ne doit être introduit sans mettre à jour cette table, les fixtures et les tests.

| `type` | `reason` | `severity` | Fatal resolver ? | Continuer les réparations ? | Message UI recommandé |
|---|---|---:|---:|---:|---|
| `link_unresolvable` | `linked_cards_fixed_on_different_rows` | `warning` | non | oui | Impossible d’aligner ce link : plusieurs cards fixes sont sur des lignes différentes. |
| `link_unresolvable` | `linked_cards_same_column` | `error` | non | oui | Impossible de lier deux cards de la même colonne. |
| `link_unresolvable` | `linked_cards_in_direct_conflict` | `error` | non | oui | Impossible de lier deux cards déjà en conflict. |
| `link_unresolvable` | `link_target_missing` | `warning` | non | oui | Un link cible une card absente. |
| `conflict_unresolvable` | `conflicted_cards_fixed_on_same_row` | `warning` | non | oui | Impossible de séparer ce conflict : les deux cards sont fixes. |
| `conflict_unresolvable` | `conflict_target_missing` | `warning` | non | oui | Un conflict cible une card absente. |
| `column_collision_unresolvable` | `same_column_collision_with_fixed_cards` | `error` | non | oui, puis layout partiel | Collision insoluble dans une colonne. |
| `invalid_action_replayed` | `fixed_card_move_refused` | `warning` | non | oui | Une ancienne action voulait déplacer une card fixe ; elle a été ignorée. |
| `invalid_action_replayed` | `lock_target_missing_row` | `warning` | non | oui | Impossible de rejouer ce lock : aucune row courante disponible. |
| `invalid_action_replayed` | `played_target_row_mismatch` | `warning` | non | oui | Une target jouée n’était plus sur la row attendue ; elle reste figée sur sa row courante. |
| `invalid_action_replayed` | `played_empty_slot_missing_target_row` | `error` | non | oui, layout partiel | Hole de plateau joué invalide : `targetResolvedRow` manquant. |
| `missing_target` | `card_target_missing` | `warning` | non | oui | Une target de transaction est absente. |
| `missing_target` | `move_target_missing` | `warning` | non | oui | Une borne de move est absente ; fallback stable appliqué. |
| `resolver_max_passes_reached` | `max_passes_reached` | `error` | oui | non | Le resolver a atteint sa limite de passes. |
| `resolver_max_passes_reached` | `max_repairs_per_pass_reached` | `error` | oui | non | Le resolver a atteint sa limite de réparations. |
| `resolver_cycle_detected` | `layout_cycle_detected` | `error` | oui | non | Oscillation détectée dans le resolver. |
| `hidden_column_constraint_ignored` | `hidden_column_not_resolved` | `info` | non | oui | Une contrainte liée à une colonne masquée a été ignorée. |
| `skip_unresolvable` | `skip_target_blocked` | `warning` | non | oui | Impossible de repousser cette appearance sans casser une contrainte dure. |
| `skip_unresolvable` | `skip_target_missing` | `warning` | non | oui | L’appearance à repousser est absente. |

`fatal resolver` signifie seulement que la boucle de résolution s’arrête pour cette exécution. Même dans ce cas, le resolver doit retourner un layout partiel complet et déterministe.
...
La projection doit transformer chaque transaction en un `transactionContext` stable avant d’appeler le resolver. Cette table est exhaustive pour les events V0 autorisés. Tout nouvel event qui peut influencer l’ordre doit ajouter une ligne ici avant implémentation.

Pour chaque ligne :

```txt
anchorCardId = card qui porte l’intention principale, ou null.
affectedCardIds = toutes les cards créées, retirées, déplacées, linkées, conflictuelles ou fixed par la transaction.
preferredResolvedRow = suggestion de départ, jamais vérité finale sauf played/locked déjà fixed.
```

| Event V0 | `intent` | `anchorCardId` | `affectedCardIds` | Champs contextuels obligatoires | Validation pré-transaction | Warnings possibles | Effet sur `previousLayout` |
|---|---|---|---|---|---|---|---|
| `appearance_moved_between` | `move` | `payload.appearanceId` | anchor + bornes valides | `afterTargetCardId`, `beforeTargetCardId` | refuser si played/locked, colonne différente | `missing_target`, `invalid_action_replayed` | seed position relative puis repair |
| `hole_moved_between` | `move` | `payload.holeId` | anchor + bornes valides | `afterTargetCardId`, `beforeTargetCardId` | refuser si played/locked, colonne différente | `missing_target`, `invalid_action_replayed` | seed position relative puis repair |
| `participation_added` | `card_created` | première appearance visible créée | toutes appearances visibles créées | `createdParticipationId`, `createdAppearanceIds`, `seedMode: end_of_visible_rounds` | instrument visible/existant | `column_collision_unresolvable` | nouvelles cards reçoivent un seed stable |
| `participation_removed` | `visibility_changed` | null | appearances futures retirées | `participationId`, `removedCardIds` | confirmation si link/played selon règles UI | `link_target_missing`, `conflict_target_missing` | retirer cards non jouées, garder played |
| `appearance_removed` | `visibility_changed` | null | target retirée | `removedCardIds` | refuser si played ; confirmer si link actif | `link_target_missing`, `conflict_target_missing` | supprimer l’entrée du layout |
| `hole_added` | `card_created` | `payload.holeId` | hole créé | `reason`, `preferredResolvedRow?`, `targetResolvedRow?` | `targetResolvedRow` obligatoire pour `played_empty_slot` | `invalid_action_replayed`, `column_collision_unresolvable` | seed à target/preferred row puis repair |
| `hole_removed` | `visibility_changed` | null | hole retiré | `removedCardIds` | refuser si played ; confirmer si link actif | `link_target_missing`, `conflict_target_missing` | supprimer l’entrée du layout |
| `link_created` | `link_created` | null | targets du link | `linkId`, `targetCardIds`, `linkStrategy` copié dans event | refuser même colonne ou conflict direct | `link_unresolvable` | aligner selon targetRow/rewrite constraints |
| `link_removed` | `neutral` | null | anciennes targets du link | `linkId`, `targetCardIds` | aucune hors existence | `link_target_missing` | préserver layout courant, pas de restore magique |
| `conflict_created` | `conflict_created` | null | targets concrètes ou participations étendues | `conflictId`, `scope`, `reason` | refuser même link group direct | `conflict_unresolvable` | séparer au coût minimal, pas “non-anchor” |
| `conflict_removed` | `neutral` | null | anciennes targets étendues | `conflictId` | aucune hors existence | `conflict_target_missing` | préserver layout courant, pas de restore magique |
| `appearance_skipped` | `skip` | `payload.appearanceId` | skipped + replacement/hole éventuel + ancien link group | `skippedAppearanceId`, `replacementCardId?`, `createdHoleId?`, `preferredResolvedRow?` | refuser si target played/locked | `skip_unresolvable`, `missing_target` | délink ponctuel cible puis push seule |
| `appearance_locked` | `lock` | `payload.appearanceId` | target | `preferredResolvedRow` depuis previousLayout | refuser si row courante absente | `invalid_action_replayed` | target devient fixed sur row courante |
| `hole_locked` | `lock` | `payload.holeId` | target | `preferredResolvedRow` depuis previousLayout | refuser si row courante absente | `invalid_action_replayed` | target devient fixed sur row courante |
| `appearance_unlocked` | `unlock` | `payload.appearanceId` | target | aucun | aucune hors existence | `missing_target` | fixed lock retiré, layout courant préservé |
| `hole_unlocked` | `unlock` | `payload.holeId` | target | aucun | aucune hors existence | `missing_target` | fixed lock retiré, layout courant préservé |
| `plateau_played` | `play` | null | all targets jouées, y compris holes `played_empty_slot` | `visualIndex?`, `playedResolvedRow`, `targets` | targets concrètes obligatoires | `invalid_action_replayed` | targets deviennent fixed sur leur row courante |
| `plateau_unplayed` | `unplay` | null | targets rejouables | `targets` | confirmation si historique à modifier | `missing_target` | played retiré, lock éventuel conservé |
| `instrument_visibility_changed` | `visibility_changed` | null | cards de la colonne visible/hidden | `columnId`, `hidden` | confirmation si links actifs | `hidden_column_constraint_ignored` | hidden retiré du resolver visible |
| `instrument_round_visibility_changed` | `card_created` | null | appearances nouvellement visibles | `columnId`, `visibleAppearanceIndexes`, `seedMode: end_of_visible_rounds` | instrument visible/existant | `column_collision_unresolvable` | nouvelles cards seedées, puis resolver libre |
| `participant_marked_left` | `visibility_changed` | null | appearances futures retirées | `participantId`, `removedFutureCardIds` | confirmation si links futurs | `link_target_missing`, `conflict_target_missing` | retirer futures non jouées, garder played |
| `jam_config_link_strategy_updated` | `neutral` | null | [] | `newStrategy` | aucune | aucun resolver warning attendu | n’affecte pas les links existants |
| `transaction_reverted` | `neutral` | null | cards affectées par la transaction annulée | `revertedTransactionId` | undo linéaire seulement | warnings de replay possibles | replay sans les events annulés puis resolver |

Noms obsolètes acceptés uniquement en migration de données locales récentes :
...
Les autres events alimentent `affectedCardIds`, `metadata` et les validations, mais ne déclenchent pas un deuxième appel resolver dans la même transaction.
...
Format canonique V0 verrouillé :
...
`appearanceId` est déterministe et dérivé de `(participationId, appearanceIndex)`. Exemple :

```txt
participationId = participation_abc
appearanceIndex = 1
→ appearanceId = appearance_participation_abc_1

appearanceIndex = 2
→ appearanceId = appearance_participation_abc_2
```

Règles complémentaires :

```txt
- `appearance_{uuid}` est interdit pour les appearances calculées/futures V0.
- Une appearance matérialisée et une appearance calculée pour la même paire `(participationId, appearanceIndex)` doivent avoir le même id.
- Ce format permet de cibler un round visible futur sans générateur aléatoire.
- Si un snapshot legacy contient des `appearance_{uuid}`, une migration doit construire un mapping déterministe avant d’entrer dans le nouveau resolver.
```
...
Cette section complète les sections 4.16 à 4.29. Elle vise à empêcher les interprétations divergentes pendant une réécriture complète du resolver.
...
### 4.29 Verrous finaux pour réécriture complète par Codex

Cette section transforme la spec en contrat de développement. Elle ne décrit pas de nouvelles intentions produit : elle verrouille la forme exacte du futur code.

#### 4.29.1 Schémas JSDoc obligatoires

Même si le frontend reste en JavaScript, le resolver doit exposer des typedef JSDoc ou un JSON Schema équivalent. Minimum obligatoire :

```js
/**
 * @typedef {Object} ResolverCard
 * @property {string} cardId
 * @property {"appearance"|"hole"} type
 * @property {string} columnId
 * @property {?string} participantId
 * @property {?string} participationId
 * @property {?string} appearanceId
 * @property {?string} holeId
 * @property {?number} appearanceIndex
 * @property {number} createdAtOrder
 * @property {number} baseOrder
 * @property {?number} previousResolvedRow
 ...
 * @property {string} cardId
 * @property {string} columnId
 * @property {number} resolvedRow
 * @property {number} visualIndex
 * @property {number} cardIndexInColumn
 */
 ...
 * @property {"move"|"card_created"|"link_created"|"conflict_created"|"skip"|"lock"|"unlock"|"play"|"unplay"|"visibility_changed"|"neutral"} intent
 * @property {?string} anchorCardId
 * @property {string[]} affectedCardIds
 * @property {?string} afterTargetCardId
 * @property {?string} beforeTargetCardId
 * @property {?number} preferredResolvedRow
 * @property {?number} playedResolvedRow
 * @property {?string} linkStrategy
 * @property {Object} metadata
 */

/**
 * @typedef {Object} ProjectionWarning
 * @property {"link_unresolvable"|"conflict_unresolvable"|"column_collision_unresolvable"|"invalid_action_replayed"|"missing_target"|"resolver_max_passes_reached"|"resolver_cycle_detected"|"hidden_column_constraint_ignored"|"skip_unresolvable"} type
...
Ces typedefs doivent vivre à côté du resolver ou dans un module `types.js` importable par les tests. Ils servent de contrat humain et de garde-fou Codex.

#### 4.29.2 Fonctions pures obligatoires

Le futur resolver doit être découpé en fonctions pures testables. Noms recommandés et responsabilités :

```txt
buildTransactionContext(events, projectionState, previousLayout)
validateTransactionBeforeApply(state, transaction)
buildVisibleCards(cards, hiddenColumnIds)
buildLinkGroups(links, visibleCards)
buildActiveConflicts(conflicts, visibleCards)
buildInitialLayout(cards, previousLayout, transactionContext)
applyTransactionIntent(layout, transactionContext)
resolveOrderAfterTransaction(input)
runResolutionPass(layout, constraints, priorities)
resolveColumnCollisions(layout, constraints, priorities)
resolveLinkMisalignments(layout, constraints, priorities)
resolveConflictViolations(layout, constraints, priorities)
normalizeVisualIndexes(layout)
buildColumns(projectedEntities, layoutByCardId, visibleResolvedRows)
```

Interdictions :

```txt
- aucune logique de resolver dans React ;
- aucune logique de resolver dans `JamTable` ou les handlers drag ;
- aucune lecture DOM ;
- aucune dépendance à Dexie/Zustand dans les fonctions pures ;
- aucun `Date.now()`, `Math.random()` ou ordre d’énumération objet JS dans une décision d’ordre.
```

#### 4.29.3 Contrat strict de `buildColumns()`

Le resolver ne rend pas l’UI. Il retourne le layout. `buildColumns()` construit les données affichables.
      ...
      rows: [
        {
          visualIndex: 1,
          resolvedRow: 1,
          cardId: "appearance_participation_voice_a_1" | null,
          card: {} | null,
...
```txt
- une ligne affichée est créée pour chaque `visibleResolvedRows` ;
- une colonne peut retourner `cardId: null` sur une ligne globale ;
- `cardId: null` signifie cellule vide visuelle, pas hole métier ;
- aucune card ne doit être affichée selon son rang local dans la colonne ;
- `buildColumns()` ne trie jamais par round, `appearanceIndex`, `positionInRound` ou `cardIndexInColumn` entrant.
```

#### 4.29.4 `end_of_visible_rounds` = seed initial, pas contrainte

Les textes produit peuvent dire “ajouter à la fin du round visible”. Cette phrase signifie uniquement :

```txt
seed initial / baseOrder de création
```

Elle ne signifie jamais :

```txt
contrainte finale du resolver
```

Règles :

```txt
- une nouvelle appearance round 1 est seedée après les round 1 visibles au moment de la création ;
- une nouvelle appearance round 2 est seedée après les round 2 visibles si le round 2 est visible ;
- ensuite le resolver peut déplacer ces cards librement pour satisfaire links/conflicts/played/locked/move utilisateur ;
- un test peut vérifier le seed initial seulement avant réparation, jamais comme ordre final garanti.
```

#### 4.29.5 Matrice de cas limites obligatoire

Ces cas doivent être couverts par tests unitaires ou fixtures golden :

| Cas | Attendu |
|---|---|
| Link entre deux locked rows différentes | `link_unresolvable`, aucune fixed ne bouge, layout complet |
| Link entre deux cards de même colonne | refus pré-transaction `link_targets_same_column` ; replay défensif warning si contourné |
| Conflict entre deux played même row | `conflict_unresolvable`, severity `warning`, aucune played ne bouge |
| Move d’une locked card | refus pré-transaction `card_locked_cannot_move` |
| Move d’une played card | refus pré-transaction `card_played_cannot_move` |
| Hidden column avec link visible-hidden | contrainte ignorée côté visible, aucune card visible déplacée |
| Round 2 placé avant round 1 | autorisé si c’est le meilleur layout |
| Link supprimé | ordre courant conservé, pas de retour magique |
| Conflict supprimé | ordre courant conservé, pas de retour magique |
| Move avec before/after absents | fallback previousLayout puis warning `missing_target` |
| Plateau joué avec cellule vide | création d’un hole `played_empty_slot` avec `targetResolvedRow` obligatoire |
| Replay même eventLog deux fois | même `layoutByCardId`, mêmes warnings dans le même ordre |

#### 4.29.6 Suppression explicite de l’ancien resolver

À supprimer ou sortir du chemin critique lors de la réécriture :

```txt
- tri par `appearanceIndex` ;
- tri `round-first` ;
- `positionInRound` comme vérité ;
- `order` / `position` comme vérité durable ;
- `plateauIndex` comme source resolver ;
- résolution directe dans `JamTable` ;
- alignement par index local de colonne ;
- conflict résolu automatiquement par déplacement de la “non-anchor” ;
- retour magique à l’ordre pré-link ou pré-conflict après suppression d’une contrainte.
```

Usage legacy autorisé :

```txt
- ces champs peuvent fournir un `baseOrder` ou une aide de migration si `previousLayout` est absent ;
- ils ne battent jamais `previousResolvedRow`, `played`, `locked`, links/conflicts actifs, ni les décisions du solver.
```

#### 4.29.7 Definition of Done de la réécriture resolver

Le dev Codex est terminé seulement si :

```txt
- `docs/order-resolution-hierarchy-spec.md` est respecté sans exception ;
- tous les typedefs/JSDoc du contrat existent ;
- les fonctions pures obligatoires existent ou sont couvertes par des équivalents explicitement nommés ;
- toutes les fixtures de `docs/order-resolution-golden-fixtures.md` passent ;
- tous les invariants de `docs/testing-strategy.md` passent ;
- aucun composant React ne calcule `resolvedRow` ;
- aucun tri round-first ne reste dans le chemin critique ;
- aucun alignement par rang local de colonne ne reste ;
- même eventLog rejoué deux fois produit le même layout et les mêmes warnings ;
- les warnings appartiennent au catalogue fermé de la section 4.3 ;
- `buildColumns()` utilise `visibleResolvedRows` et `visualIndex` global ;
- les anciennes propriétés d’ordre ne servent plus que de fallback/migration documenté.
```

#### 4.29.8 Property-based tests recommandés

En plus des fixtures golden, ajouter des tests génératifs simples sur petits tableaux.

Propriétés minimales :

```txt
- jamais deux cards visibles dans la même colonne avec le même `resolvedRow` sans warning `column_collision_unresolvable` ;
...
- tout link résoluble est aligné ;
- tout conflict résoluble est séparé ;
- aucune card visible active ne disparaît silencieusement du layout ;
- l’ordre d’entrée des arrays `cards`, `links`, `conflicts` ne change pas le résultat ;
- aucune décision ne dépend du temps courant, du DOM ou de l’ordre des propriétés d’objet ;
...
Le moteur doit avoir des tests couvrant au minimum les scénarios suivants, plus les invariants obligatoires de la section 4.15 et les fixtures canoniques de la section 4.27 et `docs/order-resolution-golden-fixtures.md`.
